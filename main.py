    ...
    def breadth_first_search(self, screen) -> List:
        ...
        while len(queue) > 0:
            ...
            for cell in neighbours:
                if (
                    0 <= cell[0] < len(self.maze_map)
                    and 0 <= cell[1] < len(self.maze_map[0])
                    and (
                        self.maze_map[cell[1]][cell[0]] == 0
                        or self.maze_map[cell[1]][cell[0]] == 3
                    )
            ...
            for col in range(len(self.maze_map[row])):
                if self.maze_map[col][row] == 2:
                    ...
                    pygame.draw.line(
                        ...
                        ((row + 1) * self.CELL_SIZE, col * self.CELL_SIZE),
                    )
                    pygame.draw.line(
                        screen,
                        color,
                        (row * self.CELL_SIZE, (col + 1) * self.CELL_SIZE),
        ...
        while self.is_running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT or (
                    event.type == pygame.KEYDOWN and pygame.key == pygame.K_ESCAPE
                ):
